...
#' @param df_buffer_loc this is a data frame inherited from DSPGrocery::Auto_Rural_Pop.
#'
#'
...
Calc_Counties_Pop <- function(states_unique,
  ...
  for (index_state in 1:length(states_unique)) {
    ...
    df_census_subset <- df_buffer_loc %>%
      filter(.data$state == states_unique[index_state])
