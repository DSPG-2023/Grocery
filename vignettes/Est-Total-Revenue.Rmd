...
The average spending per capita and the number of shoppers in each category (Primary, Secondary, Rare) for each type of market (Metro, Rural, Town) are used for estimating the revenue of the proposed stores.

## Average Per Capita Grocery Spend

The `Avg_Capita_Grocery_Spending()` function calculates the average per capita spending. The 2022 total US grocery sales data is taken from IBIS and divided by the 2022 total US population from the US Census Bureau. Since the population and grocery sales have taken 2022 data, the change for the current year must be accounted. This is adjusted by the functions `Adj_Capita_Grocery_Spend()` and `State_Adj_Capita_Grocery_Spend()` using CPI (Consumer Price Index) and RPP (Rural Price Parities), respectively.
...
Shoppers classified into 3 different populations:
