...
city_group_all_sales <- city_group_all_grocery_clean%>%
                                ...
                                ggplot(aes(x =GrpCode)) +
  geom_bar(position = "stack") +
  ggtitle("Number of grocery Stores in different classification groups") +
  xlab("City group Classification code") + 
  ylab("Number of Stores")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
storetype_grp_pop<-merge(ncg_cg_ds_counts,city_group)
...
popratio<-storetype_grp_pop%>%
  filter(pop_store_ratio!=Inf)%>%
  filter(pop_store_ratio<500)%>%
  filter(!(GrpCode %in% county))%>%
  ggplot(aes(x = reorder(Mailing_City,-pop_store_ratio), y =pop_store_ratio,fill=GrpCode)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  ggtitle("Cities with Population-grocery store ration less than 500") +
  xlab("Mailing City") + 
  ylab("Population/store ratio")
ggplotly(popratio)
```
...
scatter_pop_num_grocery <-storetype_grp_pop%>%
  ...
  filter(!(GrpCode %in% county))%>%
  plot_ly(x =~Pop20, y =~total_grocery,color=~GrpCode)
  scatter_pop_num_grocery<-scatter_pop_num_grocery %>% layout(title = 'Number of grocery stores by population')
scatter_pop_num_grocery
```
...
ncg_ds_cg_plot <-storetype_rucc_pop%>%
  ...
  geom_bar(position="dodge", stat="identity")+
    theme(axis.text.x = element_text(angle = 90, hjust = 1))+
    ggtitle("Distribution of diferrent type of store") +
    labs(subtitle = "By Mailing City")+
    xlab("Mailing_City") + 
...
```{r}
ncg_ds_cg_grpcode_plot <-storetype_rucc_pop%>%
  pivot_longer(cols =c(NCG_count,CG_count,DS_count),
                               names_to = "Store_Type",
                               values_to = "Count")

  ggplot(ncg_ds_cg_grpcode_plot, aes(x = GrpCode, y = Count, fill = Store_Type)) +
  geom_bar(position="dodge", stat="identity")+
    theme(axis.text.x = element_text(angle = 90, hjust = 1))+
    ggtitle("Distribution of diferrent type of store") +
...
```{r}
ncg_ds_cg_rucc_plot <-storetype_rucc_pop%>%
  pivot_longer(cols =c(NCG_count,CG_count,DS_count),
                               names_to = "Store_Type",
                               values_to = "Count")

  ggplot(ncg_ds_cg_rucc_plot, aes(x = factor(RUCC_2013), y = Count, fill = Store_Type)) +
  geom_bar(position="dodge", stat="identity")+
    theme(axis.text.x = element_text(angle = 90, hjust = 1))+
    ggtitle("Distribution of diferrent type of store") +
    labs(subtitle = "By RUCC Classification")+
    xlab("RUCC_Code") + 
    ylab("Number_Of Stores")
  ggplotly()
```
