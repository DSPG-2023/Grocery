...
ames_pop <- ia_towns %>% filter(NAME.x =="Ames") %>% pull(DP1_0001C)
ames_pop

mtown_pop <- ia_towns %>% filter(NAME.x =="Marshalltown") %>% pull(DP1_0001C)
mtown_pop

# Let's use State Center as the intermediate town. 

ia_towns %>% filter(NAME.x =="Ames") %>% st_centroid() -> pt_ames
ia_towns %>% filter(NAME.x == "Marshalltown") %>% st_centroid() -> pt_mtown
ia_towns %>% filter(NAME.x == "State Center") %>% st_centroid() -> pt_stateCenter

distA <- as.numeric(st_distance(pt_ames, pt_stateCenter))
distA

distB <- as.numeric(st_distance(pt_mtown, pt_stateCenter))
distB

reilly(ames_pop, mtown_pop, distA, distB)
