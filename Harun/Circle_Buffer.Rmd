...
```{r}
df_geocode <- google_geocode(address = "Main St, Medicine Bow, WY") %>%
  .$results %>%
  .$geometry %>%
  .$location
```
...
## Origin Point Buffer
```{r}
buffer_circle <- st_buffer(buffer_point_origin, dist = as.integer(ceiling(st_distance(buffer_point_origin$geometry, furthest_point$geometry))))
```
...
```{r}
all_counties <- Pull_Counties(unique(df_grocery_all$state))
all_cities <- Pull_Cities(unique(df_grocery_all$state))
```
